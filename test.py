...
class GetSubModel():
    def __init__(self):
        self.apikey = 'Er3lpPms8511fTTm'
        self.server = 'http://api.s4u.se/all/' + self.apikey + '/xml/all/'
        self.message = ''
        self.token = None
        self.hash = None
        self.size = 0
    ...
    def search_subtitles(self, filename):
        self.message = ''
        self.searchfor = os.path.basename(filename)[:-4]
	print ('test ' + self.searchfor)

        #print ("kaka " + self.LANG)
        searchdata = self.server + 'fname/' + self.searchfor
        print (searchdata)

        try:
            result = urllib2.urlopen(searchdata)
	...
	try:
	    xmlTag = dom.getElementsByTagName('rls')[1].toxml()
	    xmlData = xmlTag.replace('<rls>','').replace('</rls>','')
	except:
	    xmlData = None
	...
	#print ('test' + xmlData)
	    ...
	    print ('test' + xmlData)
            ...
            self.message = 'No results found ' + os.path.basename(filename)
    ...
    def download_subtitles(self, subtitles, choice, filename):
        ...
        if self.log_in():
            ...
            if result and result.get('status') == OK200:
                ...
                fp = gio.File(self.DEST + os.path.splitext(filename)[0] + '.' + self.LANG + '.' + subtitleFormat)
...
class GetSubController:
    ...
    def has_subtitle(self, filename):
        for ext in SUB_EXT:
	    #print (os.path.splitext(filename)[0] + "." + self.LANG + ext)
            if os.path.exists (os.path.splitext(filename)[0] + "." + self.LANG + ext):
                return True
        return False
...
def main():
    ...
    if options.LANG:
        ...
        controller.LANG = options.LANG
    else:
        controller.model.LANG = 'eng'
        controller.LANG = 'eng'
